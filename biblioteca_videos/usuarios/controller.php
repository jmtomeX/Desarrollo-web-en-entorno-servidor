...
switch ($operation) {
    case 1: // Insert **************************************************************
        ...
        $message_exito = "Registro con éxito, ya puedes acceder";

        $reg_user = $_POST['reg_user'];
        $reg_passw = $_POST['reg_passw'];
        $passw_repeat = $_POST['passw_repeat'];
        $id = insertUser($reg_user, $reg_passw,$passw_repeat);
        if ($id == false) {
            $msg = $message_fracaso;
...
// Funciones ***********************************************************
function insertUser($reg_user, $reg_passw, $passw_repeat)
{
    // Comprobar contraseñas
    if ($reg_passw == $passw_repeat) {
        ...
        header("Location:./registro.php?msg=Las contraseñas no coinciden");
        exit;
    }
